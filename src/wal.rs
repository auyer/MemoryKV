...
impl std::fmt::Display for KVWAL{
    fn fmt(&self, f: &mut std::fmt::Formatter) -> std::fmt::Result {
        match self {
            KVWAL::Insert { key, value } => write!(f, "Insert {{ key: {}, value: {} }}", key, String::from_utf8_lossy(&value)),
...
impl KVWAL{
    pub fn to_bytes(&self) -> Bytes {
        match self {
            KVWAL::Insert { key, value } => Bytes::from(format!("Insert {{ key: {}, value: {} }}", key, String::from_utf8_lossy(&value))),
            ...
            KVWAL::ListPrefix{ prefix }=> Bytes::from(format!("ListPrefix {{ prefix: {}}}", prefix)),
            KVWAL::Heartbeat { addr }=> Bytes::from(format!("Heartbeat from {}", addr.to_string())),
        }
    }
}
